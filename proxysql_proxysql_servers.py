...
version_added: "2.5"
author: "Max Bubenick (@maxbube)"
short_description: Adds or removes proxysql hosts from proxysql cluster.
description:
...
options:
  ...
  port:
    description:
      - The port at which the proxysql admin instance can be contacted.
    default: 6032
  weight:
    description:
  ...
  save_to_disk:
    description:
      - Save proxysql servers config to sqlite db on disk to persist the
        configuration.
    default: True
  load_to_runtime:
    description:
      - Dynamically load proxysql host config to runtime memory.
    default: True
  login_user:
    description:
...
RETURN = '''
stdout:
    description: The proxysql server modified or removed from proxysql
